...
<form name="form1" id="form1" method="post" action="">
    搜索: 用户名：<input name="yonghuming" type="text" id="yonghuming"/> 姓名：<input name="xingming" type="text" id="xingming"/>
    QQ：<input name="QQ" type="text" id="QQ"/> 邮箱：<input name="youxiang" type="text" id="youxiang"/> 电话：<input
        name="dianhua" type="text" id="dianhua"/> 身份证：<input name="shenfenzheng" type="text" id="shenfenzheng"/>
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF"
    ...
    <tr>
        ...
        <td><a href="sh.jsp?id=<%=id%>&tablename=yonghuzhuce&yuan=<%=issh%>"><%=issh %>
        </a></td>
        <td><%=beizhu %>
        </td>
        <td width="138" align="center"><%=addtime %>
        </td>
        <td width="90" align="center"><a href="yonghuzhuce_updt.jsp?id=<%=id%>">修改</a> <a
                href="del.jsp?id=<%=id %>&tablename=yonghuzhuce" onClick="return confirm('真的要删除？')">删除</a> <a
                href="yonghuzhuce_detail.jsp?id=<%=id%>" target="_blank">详细</a></td>
    </tr>
    <%
        }
    %>
...
<p align="center">&nbsp;共<%=zgs%>条记录&nbsp;&nbsp;<%=page_record %>条/页　<a href="yonghuzhuce_list.jsp?page=1">首页</a>　<a
        href="yonghuzhuce_list.jsp?page=<%= curpage-1%>">上一页</a>　<A
        href="yonghuzhuce_list.jsp?page=<%= curpage+1%>">下一页</A>　<a href="yonghuzhuce_list.jsp?page=<%=zys %>">尾页</A>　当前第<FONT
        color=red><%=curpage %>
